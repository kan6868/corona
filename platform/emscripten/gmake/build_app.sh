...
pushd $path > /dev/null
	...
	echo '\t' make AR=emar CC=emcc CXX=em++ verbose=1 config="$CONFIG" CXXFLAGS="-s STRIP_UNUSED_IMPORTS=1 -s USE_SDL=2"
	make AR=emar CC=emcc CXX=em++ verbose=1 config="$CONFIG" CXXFLAGS="-s STRIP_UNUSED_IMPORTS=1 -s USE_SDL=2 -I\"$path/hack_includes\""
	checkError
